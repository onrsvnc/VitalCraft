...
public class RoadPlacementModificationHelper : StructureModificationHelper
{
    ...
    public bool roadBuildingState = false;
    Vector3 roadBuildingStartPosition;
    ...
    public override void PrepareStructureForModification(Vector3 inputPosition, string structureName, StructureType structureType)
    {
        ...
        if(gridPosition == roadBuildingStartPosition) return;
        roadBuildingStartPosition = gridPosition;
        if (grid.IsCellTaken(gridPosition) == false)
        {
            ...
            if (structuresToBeModified.ContainsKey(gridPositionInt)) { roadBuildingState = false; }
            if (structuresToBeModified.ContainsKey(gridPositionInt) && roadBuildingState == false)
            {
                RevokeRoadPlacementAt(gridPositionInt);
                resourceManager.AddMoney(structureData.placementCost);
            }
            else if (resourceManager.CanIBuyIt(structureData.placementCost) && !structuresToBeModified.ContainsKey(gridPositionInt))
            {
                roadBuildingState = true;
